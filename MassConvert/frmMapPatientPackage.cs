...
{
    public partial class frmMapPatientPackage : Telerik.WinControls.UI.RadForm
    {
        ExcData exc;
        SQL db;
        BindingSource bs = new BindingSource();
        public frmMapPatientPackage()
        {
            InitializeComponent();
        }
        ...
        private void frmMapPatientPackage_Load(object sender, EventArgs e)
        {
            ...
            for (int i = 1; i <= gvPatient.Rows.Count; i++)
            {
                if (Convert.ToBoolean(gvPatient.Rows[i - 1].Cells["Check"].Value) == true)
                {
                    ...
                    for (int j = 0; j <= dtProMapPackage.Rows.Count - 1; j++)
                    {
                        ...
                        //Show status
                        lblStatus.Text = "Map Patient : " + Forename + " " + Surname + " vs Package : " + PackageName ;
